...
struct radix_tree_node {
	...
	/* 结点标签数组=每个slot需要的最大标签位数（RADIX_TREE_MAX_TAGS=2）*slot数所需的long类型变量数（RADIX_TREE_TAG_LONGS=64） */
	/*目前RADIX_TREE_MAX_TAGS为2，表示只记录两个标志，
...
/*
 * is_slot == 1 : search for the slot.查找slot
 * is_slot == 0 : search for the node.查找node
 */
/* 参数index是文件中字节偏移量，而页缓存是页偏移量，需要进行转换，页偏移量 = index >> SHFIT */
 ...
 *	@root:		radix tree root  radix树根节点
 *	@index:		index key  指定偏移量
 *
 ...
 *	item is a bug.
 */
/* 设置radix树中页的标志；包括从根节点到叶子结点的路径中的所有节点，都被加上了标记 */
 ...
 */
 /* 清除radix树中页的标志；包括从根节点到叶子结点的路径中的所有节点，都被清除了标记 */
			...
			unsigned long index, unsigned int tag)
{
	 ...
	 */
	 /* 从树根开始，到叶子节点，建立描述路径的数组radix_tree_path,数组中的每一个对象都对应一个路径中的一个节点 */
