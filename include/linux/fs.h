...
/*
	页高速缓存，对于每一个打开的文件，只有一个address_space对象，但是可以有许多个vm_area_struct结构。
用于管理文件（struct inode)映射到内存的页面(struct page)的
...
*/
struct address_space {
	struct inode		*host;		/* 指向拥有该对象的inode索引节点的指针*/
	struct radix_tree_root	page_tree;	/* 包含全部页面的radix树*/
	spinlock_t		tree_lock;	/* 保护radix tree的锁*/
	...
	spinlock_t		i_mmap_lock;	/*  保护radix优先搜索树的锁*/
	...
	struct list_head	private_list;	/* ditto 与索引节点相关的间接块的脏缓冲区的链表*/
